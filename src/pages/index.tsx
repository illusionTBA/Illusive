...
const Home: NextPage = () => {
  ...
  if (typeof window !== "undefined") {
    if (!localStorage.getItem("ill@history"))
      localStorage.setItem("ill@history", "[]");

    const addToHistory = (tab: any) => {
      var h = document.getElementsByClassName(styles["historyapps"])[0];
      var newTab = document.createElement("div");

      h.appendChild(newTab);

      var ls = JSON.parse(gethistory as any);
    ...
    window.onload = function (e) {
      ...
      contextElement.addEventListener(
        "mouseover",
        () => (contextElement.over = true)
      );
      contextElement.addEventListener(
        "mouseout",
        () => (contextElement.over = false)
      );
          ...
          if (Paths[path]) {
            ...
            if (!urlbar.isfocus && frame.loaded) {
              ...
              if (toSet.startsWith("/service/")) {
                ...
                urlbar.innerHTML = `<span class="${styles.marking}">${
                  toSet.protocol
                }//</span>${toSet.href.replace(toSet.protocol + "//", "")}`;
              }
            }
          }
  ...
  return (
    <div className={styles.main} data-theme="classic">
      <Head>
        ...
        <meta
          name="description"
          content="Illusive | Gateway to Evading Censorship"
        />
      ...
      <main className={styles.main}>
        <div id={styles["context-menu"]}>
          ...
          <div className={styles.item}>
            <i className={`${styles.uil} ${styles["uil-redo"]}`}></i>Refresh
          </div>
          <div className={styles.item}>
            ...
            {history.map((tab) => {
              return (
                ...
                >
                  ...
                  <img
                    src={"https://www.google.com/s2/favicons?domain=" + tab.uri}
                  ></img>
                </div>
              );
            })}
        ...
        <div id="apps" className={styles["apps"]}>
          {apps.map((app) => {
            ...
            if (app.type == "sep") {
              return <div key={app.type} className={styles.separator}></div>;
            }
            if (app.type == "app") {
              if (!app.action) {
                return (
                  <a key={app.name} onClick={cancelEvent}>
                    <div
                      key={app.name}
                      className={`${styles.app} ${styles.normalLink}`}
                      onClick={(e) =>
                        openApplication(app.action, app.url, Router)
                      }
                    >
                      <img
                        style={{
                          filter: app.filter || "invert(0)",
                          width: app.size + "px" || "30px",
                          height: app.size + "px" || "30px",
                        }}
                        src={app.image}
                        alt={app.name}
                      ></img>
        ...
        <div className={styles["apps-frame"]}>
          <div
            id="dropdown"
            onClick={toggleDrop}
            className={`${styles["apps-dropdown"]} dropdown`}
          >
            <IoChevronDown />
          </div>
          ...
          <div
            id="main-frame"
            className={`${styles["main-frame"]} ${styles["apps-down"]}`}
          >
        ...
        <div className={styles["bottom-container"]}>
          <div className={styles["bottom-left-menu"]}>
            <MdTab
              id="tabby"
              className={`${styles["open-tabs"]} ${styles["fillS"]}`}
              onClick={(e) => {
                toggleTabs();
              }}
              width="25"
          ...
          <div className={styles["user-form"]}>
            <img
              onClick={(e) => {
                openApplication("home", "/home", Router);
              }}
              src="/favicon.ico"
              className={`${styles["icon-ico"]}`}
            ></img>
            <div
              contentEditable="true"
              className={styles["form-submit-main"]}
              id="urlbar"
            ></div>
          ...
          <div className={styles["bottom-right-menu"]}>
            <svg
              width="16"
              height="16"
              className={`${styles["main-nav-icon"]} ${styles["fill"]}`}
              viewBox="0 0 35 30"
              fill="none"
            ...
            >
              <path
                fillRule="evenodd"
                clipRule="evenodd"
                d="M15 0C6.71576 0 0 6.71573 0 15C0 23.2843 6.7157 30 15 30H29.1667C32.3883 30 35 27.3883 35 24.1667V5.83334C35 2.61166 32.3883 0 29.1667 0H15ZM15.9393 21.9393C15.3536 22.5251 15.3536 23.4749 15.9393 24.0607C16.5251 24.6465 17.4749 24.6465 18.0607 24.0607L26.0607 16.0607C26.6464 15.4749 26.6464 14.5251 26.0607 13.9393L18.0607 5.93933C17.4749 5.35355 16.5251 5.35355 15.9393 5.93933C15.3536 6.52512 15.3536 7.47488 15.9393 8.06067L22.8787 15L15.9393 21.9393Z"
                fill="#D9D9D9"
              />
            </svg>
            <IoReloadOutline
              id="reload-frame"
              className={`${styles["main-nav-icon"]} ${styles["reload-btn"]}`}
